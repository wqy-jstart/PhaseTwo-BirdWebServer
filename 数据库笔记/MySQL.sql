...
ALTER TABLE hero
    ADD gender VARCHAR(32) AFTER username;
...
VALUES (1, '张三', '1992-08-02 11:22:05', 5000.59);

#插入李四,DATETIME类型在不指定时分秒,默认为00:00:00
INSERT INTO userinfo
VALUES (2, '李四', '1999-05-16', 6500.23);
...
INSERT INTO userinfo
VALUES (3, '王五', '1987-04-13 06:15:55', 1900.788);
INSERT INTO userinfo
VALUES (4, '赵六', '1966-03-24', 2000);
INSERT INTO userinfo
VALUES (5, '钱七', '1860-01-22', 40000);
...
SELECT name, dept_id, sal
FROM emp
ORDER BY dept_id DESC, sal DESC;
#部门降序,工资降序排列
...
SELECT MIN(sal) 最低工资, MAX(sal) 最高工资, AVG(sal) 平均工资, SUM(sal) 工资总和
FROM emp;

#聚合函数忽略NULL值,在AVG中比较明显可以看出这一点,一下仅对4个奖金的人取了平均值,并非11个人
SELECT MIN(comm) 最低奖金, MAX(comm) 最高奖金, AVG(comm) 平均奖金, SUM(comm) 奖金总和
FROM emp;
...
SELECT *
FROM emp_dept_sal;#查看emp_dept_sal的所有数据
...
FROM emp e,
     dept d,
     (SELECT d.loc, MAX(e.sal) max #子查询表
      FROM emp e,
           dept d
      WHERE e.dept_id = d.id
      GROUP BY d.loc) a
...
#修改表中的某个字段
ALTER TABLE article
    CHANGE title title VARCHAR(300);
ALTER TABLE article
    CHANGE content content VARCHAR(3000);
...
INSERT INTO article(title, content, u_id)
VALUES ('标题', 'haode', userinfo.id);
...
VALUES ('刘德华', '男', '亲戚'),
       ...
       ('貂蝉', '女', '同事');
insert into trade
values (null, 1000, '微信', '2021-03-20', 1),
